...
- **viewer.py** — renders the transcript viewer page (pure string → HTML, no I/O): an `<audio>` element, the segment list below it color-coded by speaker and clickable to seek, the currently-playing segment auto-highlighted via `timeupdate`, and a "rename" link on each unlabeled alias that POSTs to `/api/rename` and reloads on success.
- **webserver.py** — a small `ThreadingHTTPServer` (binds `127.0.0.1` only) that makes `viewer.py`'s output usable in a real browser. `GET /view/<id>` renders the page fresh from the current transcript JSON each time; `GET /audio/<id>` streams the source recording directly (resolves `source_file` from the transcript, not an arbitrary path — the exact file served is scoped to what's already in the manifest); `POST /api/rename` calls the same `review.py` rename/enroll path `vmt review` uses, under a lock (single-writer safety), so the browser and the CLI can never diverge into separate sources of truth. `file://` audio URLs were tried first and dropped: browsers block a local page from loading `file://` resources outside its own directory, which the audio and the HTML never share. The embedding model is cached at module level in `speakers.py` across requests — reloading it (a multi-second op) per rename inside a long-lived server made every rename feel stuck.
- **config.py** — resolves settings from `~/.config/vmt/config.toml`, falling back to env vars (`VMT_DATA_DIR`, `HF_TOKEN`, `VMT_WHISPER_MODEL`, `VMT_SIMILARITY_THRESHOLD`, `VMT_VIEWER_PORT`) and then defaults. Default data dir: `<repo>/data`.
